...
def train_global_bank_cem(
    ...
    with enter_obj as run:
        ...
        if (not rerun) and os.path.exists(load_file_name):
            ...
            if concept_epochs:
                print(
                    f"\tTraining up the global concept generator for {concept_epochs} epochs"
                )
                ...
                if (not rerun) and "concept_model_path" in config and (
                    ...
                    if "concept_model_path" in config:
                        concept_model_saved_path = os.path.join(
                            result_dir or ".",
                            f'{config["concept_model_path"]}.pt'
                        )
            ...
            ####################################################################
            ## Step 2: Train the entire model end-to-end
            ####################################################################

            start_time = time.time()
            joint_epochs = config['max_epochs']
            print(
