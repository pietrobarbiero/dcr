...
for i, dataset in enumerate(datasets):
    ...
    for g in groups:
        ...
        sep += 0.5
        ymin = min(ymin, round(accs_mean.min()-10, -1))
...
model_ordered = model_df['model'].array[[0,2,3,4,1,6,7,8,5]]
fig.legend(patches, model_ordered, loc='upper center',
          # bbox_to_anchor=(-0.8, -0.2),
          bbox_to_anchor=(0.5, 0.05),
           fontsize=14, frameon=False,
           fancybox=False, shadow=False, ncol=len(models)/2 + 1)
plt.tight_layout()
