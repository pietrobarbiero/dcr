...
class XDecisionTreeClassifier(BaseClassifier, BaseXModel):
    ...
    def get_local_explanation(self, x:torch.Tensor, y:torch.Tensor, concept_names: list = None, *args,
                              return_time: bool = False, **kwargs):
        y = y.squeeze()
        target_class = y if len(y.shape) == 0 else y.argmax()
        if concept_names is None:
            concept_names = [f"f_{i}" for i in range(self.n_features)]
        start_time = time.time()
        if self.explanations[target_class] != "":
            global_explanation = self.explanations[target_class]
        else:
            global_explanation = tree_to_formula(self.model, concept_names, target_class)
            self.explanations[target_class] = sympy.to_dnf(global_explanation)
        list_local_explanations = global_explanation.split("|")
        explanation = ""
