...
class F1Score(Metric):
    ...
    def __call__(self, outputs: torch.Tensor, targets: torch.Tensor) -> float:
        ...
        if len(outputs.squeeze().shape) > 1 and len(targets.squeeze().shape) == 1:
            discrete_output = outputs.argmax(dim=1).numpy()
        # Multi-label or Binary classification
        else:
            discrete_output = outputs.cpu().numpy() > 0.5
            targets = targets > 0.5
        targets = targets.cpu().numpy()
        f1_val = f1_score(targets, discrete_output, average=self.average, zero_division=0) * 100
        return f1_val


class RocAUC(Metric):
    """
    ROC AUC score computed on the predictions of a model and the actual labels.
    """
    ...
    def __call__(self, outputs: torch.Tensor, targets: torch.Tensor) -> float:
        ...
        # Multi-label or Binary classification
            ...
            targets = targets > 0.5
        targets = targets.cpu().numpy()
        auc_score = roc_auc_score(targets, discrete_output, multi_class="ovr")
        return auc_score
